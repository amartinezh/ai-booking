...
export class Hl7FhirService {
  ...
  async getPatientSummaryBundle(patientId: string): Promise<any> {
    ...
    const fhirPatient = {
      ...
      id: patientConfig.id,
      identifier: [
        { system: 'urn:oid:2.16.170.1.3.1.2.1.1', value: patientConfig.cedula },
      ],
      ...
      managingOrganization: patientConfig.eps
        ? { display: patientConfig.eps.name }
        : undefined,
    };
    ...
    for (const record of patientConfig.clinicalRecords) {
        ...
        section: [
          {
            ...
            text: {
              status: 'generated',
              div: `<div>${record.chiefComplaint}</div>`,
            },
          },
          {
            ...
            text: {
              status: 'generated',
              div: `<div>${record.currentIllness}</div>`,
            },
          },
          {
            ...
            text: {
              status: 'generated',
              div: `<div>${record.evolutionNotes}</div>`,
            },
          },
        ],
      };
      entries.push({
        fullUrl: `Composition/${record.id}`,
      ...
      if (record.vitalSigns) {
        if (record.vitalSigns.heartRate) {
          entries.push({
            fullUrl: `Observation/${record.vitalSigns.id}-hr`,
            resource: {
