...
export default function EpsClient({ data }: { data: any[] }) {
    ...
    const handleSearch = useDebouncedCallback((term: string) => {
        ...
        router.replace(`/dashboard/eps?${params.toString()}`);
    }, 300);
    ...
    return (
        <div className="space-y-6">
            ...
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                <div>
                    ...
                    <p className="text-sm text-gray-500 mt-1">
                        Gestione los convenios de salud válidos para asignación de cupos y triage.
                    </p>
                </div>
            ...
            {/* Creación */}
            {isModalOpen && <EpsModal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} />}
        </div>
    );
}
