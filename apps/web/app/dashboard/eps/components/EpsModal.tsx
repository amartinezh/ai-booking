...
export default function EpsModal({ isOpen, onClose }: { isOpen: boolean; onClose: () => void }) {
    const [state, action, pending] = useActionState(createEps, null);

    // Cerrar el modal exitosamente si se guardó
    useEffect(() => {
        if (state?.success) {
            onClose();
        }
                    ...
                    <div className="space-y-4">
                        <div>
                            ...
                            <input
                                ...
                                name="name"
                                id="name"
                                placeholder="Ej. Sura, Sanitas, Mutual Ser..."
                                required
                        ...
                        <div>
                            ...
                            <input
                                ...
                                name="nit"
                                id="nit"
                                placeholder="Ej. 800.222.111-9"
                                className="w-full rounded-md border border-gray-300 px-3 py-2 text-gray-900 focus:border-indigo-500 focus:outline-none focus:ring-1 focus:ring-indigo-500 sm:text-sm"
                            />
                            {state?.issues?.nit && <p className="mt-1 text-xs text-red-500">{state.issues.nit}</p>}
                        </div>
                        ...
                        <div className="flex items-start bg-gray-50 p-3 rounded-lg border border-gray-100">
                            ...
                            <div className="ml-3 text-sm">
                                <label htmlFor="isActive" className="font-medium text-gray-700">Convenio Activo Inmediatamente</label>
                                <p className="text-gray-500 text-xs">Si desmarcas esto, la Inteligencia Artificial no la ofrecerá para agendar cupos.</p>
                            </div>
                        </div>
                    </div>
