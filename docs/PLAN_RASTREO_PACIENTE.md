...
4. ~~**Retención** de `PatientLookupLog` e `InteractionLog`.~~ ✅ Decidido y hecho el 2026-09-21 (Ley 1581, Habeas Data): las conversaciones del bot (`InteractionLog`) se conservan **180 días** —cubren de sobra una PQRS y una queja tardía ante la Superintendencia— y la bitácora de quién consultó a quién en el rastreo (`PatientLookupLog`) **365 días**: es un registro de acceso y vive más que lo consultado, para poder responder meses después «¿quién miró mis datos?». Pasado el plazo las filas se **borran** (nada lee tan atrás). La purga es un cron de la API (`RetentionService`, 3:30 a. m. de Colombia), por lotes de 5.000 y con tope por noche, y deja constancia en SystemLog (`DATA_RETENTION_PURGE`). Los plazos viven en `@agenia/shared` (`RETENCION_DATOS`); se pueden cambiar con `RETENCION_CONVERSACIONES_DIAS` y `RETENCION_BITACORA_RASTREO_DIAS`, pero nunca por debajo de 30 días (un error de tipeo no borra lo de ayer). `HisLookupRequest` ya se purgaba a los 15 min.
...
7. ~~**Fuera de la ventana de 24 h de Meta** un mensaje libre no sale: el "enviar confirmación" del escenario B tendría que usar una plantilla.~~ ✅ Hecho el 2026-09-21. El botón no existía: el veredicto lo sugería y el personal no tenía con qué. Ahora el rastreo B ofrece **«Enviar confirmación por WhatsApp»** cuando el veredicto es `CITA_DEL_HIS_NO_ESPEJADA`. Dentro de la ventana de 24 h sale como texto; fuera, con la plantilla nueva **`HIS_APPOINTMENT_CONFIRMATION`** (nombre, servicio, médico, fecha y hora; sin ella aprobada no sale y la pantalla lo dice). La API (`POST /appointments/his-confirmation`, ORG_ADMIN y BOOKING_AGENT) comprueba que el cupo sea de la clínica, futuro, ocupado por el hospital y **sin** cita de AgenIA; envía **solo al WhatsApp que AgenIA ya tiene** del paciente (nunca a un número escrito en pantalla); aplica el alcance del agente, y no repite en 10 min. Si la consulta en vivo no confirmó que la cita es de ese paciente, quien atiende tiene que afirmarlo tras mirarlo en el HIS; queda anotado cuál de las dos fue, en la bitácora del rastreo (`queryKind = CONFIRM`) y en SystemLog. El texto le dice al paciente que la cita la asignó el hospital y que para cancelarla o cambiarla debe comunicarse con el hospital: el bot no la conoce.
...
10. ~~**`updateAttendance` y el recordatorio manual tampoco aplican el alcance del agente.**~~ ✅ Corregido el 2026-09-20: las dos acciones leen la cita y aplican `alcanceDeLaSesion` + `citaFueraDeAlcance` antes de escribir o de enviar.
    - ~~**10b.** El endpoint `POST /appointments/:id/send-manual-reminder` de la **API** comprobaba solo rol y tenant.~~ ✅ Corregido el 2026-09-21: la regla pura (`citaFueraDeAlcance` y `AlcanceAgente`) se mudó a **`@agenia/shared`** y ahora la aplican los dos lados. El actor sale del token (`@CurrentUser`), nunca del body. Falla **cerrado**: si el perfil no se puede leer, no se envía. La comprobación de la web se conserva —ahorra el viaje y da un mensaje claro—, pero la frontera real es la de la API, porque el endpoint se puede llamar directo con un token.
...
13. ~~**Canal de correo para el aviso al agendador.**~~ ✅ Decidido el 2026-09-21: **por ahora el aviso es solo por WhatsApp**; no se maneja correo. La columna `agendadorEmail` se conserva (sin uso) para no migrar el día que se decida un transporte.
14. ~~**Qué pasa cuando el agendador no lee el aviso.**~~ ✅ Hecho el 2026-09-21. La señal es que **nadie tomó la excepción en la bandeja**, no el «leído» de Meta (leer no es ocuparse, y el leído depende de que el agendador lo tenga activado). Si pasan **30 min** tras el aviso sin que nadie la tome, se **recuerda**, hasta **2 veces** por gravedad (si la gravedad sube, es un aviso nuevo y la cuenta vuelve a cero). El recordatorio va al agendador **y a un número de respaldo** opcional (`agendadorRespaldoWhatsapp`, se configura en la bandeja junto al del agendador): ese es el escalamiento. Usa la misma plantilla `SYNC_EXCEPTION_ALERT`, con la causa empezando por «RECORDATORIO 1 de 2: nadie la ha tomado en la bandeja», así que no hay que aprobar otra en Meta. Dos réplicas no recuerdan dos veces (compare-and-set sobre la hora del último envío y la cuenta; verificado con dos conexiones contra Postgres real). En el historial queda «Se le recordó al agendador».
15. ~~**Cita cuya hora ya pasó sin llegar al hospital.**~~ ✅ Hecho el 2026-09-21. Estado nuevo **`VENCIDA`** («Venció sin resolución»): el vigilante la cierra cuando la hora de la cita pasó hace **más de 7 días** y **nadie la movió en 7 días** (tomarla, soltarla o reabrirla le da otra semana: quien la está trabajando no la pierde). No es `AUTO_RESUELTA`, porque el problema no se resolvió, y es **firme**: el vigilante no la reabre sola; una persona sí puede reabrirla. Aplica a toda excepción con hora de cita.
...
17. **Citas con `FE_HORA_CIT` ilegible y el veredicto `NO_ESTA_EN_EL_HIS`** (hallado al medir la Fase 2, 2026-09-20). El HIS guarda parte de las horas en un formato que no cumple `'YYYY/MM/DD HH:MM'` (`MAPEO_HIS.md` §2.1: 5,7 % de las citas elaboradas en 30 días; valores como `'2026/08/29 1'`). Consecuencias, las dos malas:
    ...
    ✅ **Mitad cerrada el 2026-09-21** (la que no dependía de ninguna decisión): el recorte del agente ya **no se pierde** en la consulta por cupo —`resolverRespuestaHis` lo descartaba— y viaja hasta el veredicto. Ahora:
    - una respuesta incompleta **NO** produce `NO_ESTA_EN_EL_HIS`, sino el veredicto nuevo **`HORA_ILEGIBLE_EN_EL_HIS`** («el hospital respondió algo que no se pudo leer»), que dice lo que no se sabe y manda a mirarlo en la aplicación del hospital;
    - en el escenario A ya no se acusa una deriva (`ENTREGADA_PERO_AUSENTE`): queda «sin verificar», diciendo por qué;
