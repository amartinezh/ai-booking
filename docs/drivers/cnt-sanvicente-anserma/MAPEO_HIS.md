...
1. ✅ **Formato confirmado (bloque 5):** `FE_HORA_CIT` = fecha+hora completa con **barras**: `'YYYY/MM/DD HH:MM'` (16 chars, 24h, cero a la izquierda; ej. `'2026/10/17 09:20'`). Convive data legada sucia (longitudes 12/13, incluso `'31'` — e incluso una fila de 2026 con `'2026/08/29 1'`): el **lector** del agente debe ser tolerante; el **escritor**, estricto al formato de 16 chars. 📊 **Medido el 2026-09-03: el 5,7 % (419 de 7.403 citas elaboradas en 30 días) tiene la hora ilegible.** ✅ El lector NO era tolerante y lanzaba, tumbando `detectChanges` y `snapshotAppointments` enteros con una sola fila — corregido con `feHoraCitAIsoOrNull()`, que omite la fila y la cuenta. ⚠️ Además, el regex comprobaba la forma pero no el RANGO: `2026/13/45 10:00` desbordaba en silencio hasta el 14 de febrero de 2027. Ahora se valida por ida y vuelta. El INSERT de prueba con guiones pasó constraints pero NO es el formato de la app — jamás escribir con guiones. |
...
- ✅ **Tabla DERIVADA de los datos, no supuesta (2026-09-02).** La sección D de `sql/PENDIENTE_CORRER_EN_HOSPITAL.sql` midió el convenio dominante de cada combinación EPS + régimen + PyP sobre 90 días de citas reales:

  | EPS | Régimen | Servicio | Convenio | Cuota |
  |---|---|---|---|---|
  | Sura | subsidiado | normal / PyP | 467 `SUBS` | 84,5 % / 94,3 % |
  | Sura | contributivo | normal / PyP | 473 `CONTRIBUTIVO` | 84,8 % / 88,0 % |
  | Nueva EPS | subsidiado | normal | 283 `NUEVASUBSID` | 89,6 % |
  | Nueva EPS | subsidiado | **PyP** | 489 `PYPSUBS` | 94,4 % |
  | Nueva EPS | contributivo | normal / PyP | 473 `CONTRIBUTIVO` | 73,4 % / 65,6 % |

- 🚨 **Los NIT estaban CRUZADOS.** `800088702` es **EPS Suramericana** y `900156264` es **Nueva EPS** — así lo dice la tabla `EPS` del hospital y así son los NIT públicos. AgenIA los tenía al revés y `mapping.json` repetía el cruce, de modo que los dos errores se cancelaban. Corregidos los dos a la vez; van juntos o la facturación se voltea.
- 🚨 **PyP depende del RÉGIMEN, no solo de la EPS.** La clave era `${nit}|PYP` y se aplicaba a cualquier régimen: un contributivo de Nueva EPS con servicio de PyP se facturaba al 489 `PYPSUBS`, un contrato subsidiado. Ahora es `${nit}|${REGIMEN}|PYP`, con caída al convenio del régimen cuando no existe (que es lo que hace el hospital con Sura).
- **Códigos de régimen del hospital** (`CD_CODI_REG_RPE`): más finos que subsidiado/contributivo. `01` y `02` se comportan como SUBSIDIADO; `07`–`12` como CONTRIBUTIVO. Inferido de los convenios que usan, no de un catálogo — ver la consulta D.6.
- ✅ **CONFIRMADA CONTRA EL CATÁLOGO OFICIAL (2026-09-02).** `REGIMEN` se une a `TIPO_REGIMEN_RESOL4505` por `TX_CODI_RTT_REG` (el tipo de régimen de la Resolución 4505): `01`/`02`/`14` son tipo 2 SUBSIDIADO; `07`–`12` y `18` son tipo 1 CONTRIBUTIVO. Cruzando eso con el convenio dominante observado, **las 18 combinaciones (EPS × régimen) concuerdan sin una sola excepción**: todo régimen subsidiado lleva a 467/283/489 y todo contributivo a 473. Ya no es una lectura de los nombres de los convenios: es el catálogo del hospital confirmándolo de forma independiente. (`P` = OTRO y `F` = FOSYGA son tipos NO ASEGURADO y EXCEPCIÓN — no son afiliaciones de EPS, por eso no aparecen en estas citas.)
- ✅ **El fan-out de `R_PAC_EPS` no invalida la tabla.** Es many-to-many y ensucia la cola de cada grupo (por eso Nueva EPS contributivo sale al 73 % y no al 85-94 %), pero solo puede **inflar** un conteo, nunca esconderlo: sumando por convenio se obtiene una cota superior del uso real, y la de 290 `NUEVAEPSCONT` —el único candidato alternativo— es de **dos** citas en 90 días. El hospital no lo usa. Deduplicar por "paciente con una sola afiliación" no es posible: esa consulta devuelve cero filas, todo paciente acumula varias.
...
- ✅ **`CITAS_ANULADAS`** (descubierta por la prueba manual del bloque 17, §2.1bis) — **archivo de auditoría de cancelaciones**, no un catálogo de agenda. **Esquema completo confirmado (24 columnas; volcado columna a columna el 2026-09-01, bloque 28):** ⚠️ **NO es `CITAS_MEDICAS` con el sufijo cambiado**, aunque se le parezca — esa lectura, que estuvo aquí escrita, hizo que el driver copiara cuatro columnas inexistentes (`NU_ESTA_CIAN`, `CD_CODI_CECO_CIAN`, `CD_CODI_LUAT_CIAN`, `FE_SOLI_CIAN`) y **rompiera toda cancelación en el hospital** con "Invalid column name". Tiene la mayoría de los campos de la cita, le faltan esos cuatro, y suma `CD_CODI_MOTI_CIAN` varchar(2) (código de motivo → catálogo `dbo.MOTIVOANUL`), `TX_OBSE_CIAN` varchar(255) (observaciones libres), y `NU_CONE_ANUL_CIAN` (consecutivo propio de la anulación, distinto del `NU_NUME_CONE_CIAN` heredado de la cita original).
  ...
  **✅ Catálogo `dbo.MOTIVOANUL` — completo (23 motivos, confirmado 2026-08-23):**
  ...
  | `NA` | **NO ASISTIO** | 285 histórico / **4 en los últimos 365 días** (I.8, 2026-09-07) | ❌ **La lectura que había aquí era exactamente la contraria a la verdad, y costó tres semanas.** Decía: *"el no-show también pasa por DELETE+archivo en CITAS_ANULADAS, no por un NU_ESTA_CIT distinto; esto reduce la importancia de descifrar el estado 2"*. Al medirlo, `NA` se usa **cuatro veces al año** mientras el estado `2` recibe **~16.800 filas al año**: cuatro mil doscientas veces más. Si el no-show viviera aquí, este hospital tendría cuatro inasistencias anuales. **`NA` no es el no-show del hospital — es una rareza.** Y al eliminar el único sitio alternativo, esto no reduce sino que **dispara** la importancia del estado `2`, que es donde el no-show tiene que estar. |
  ...
  **⚠️ Corrección (2026-09-06): esta nota rebajaba el estado `2` a "curiosidad menor" y estaba equivocada en los dos extremos.**

  Lo que decía: que con `NA` confirmado dentro de `CITAS_ANULADAS` la hipótesis de que `NU_ESTA_CIT=2` fuera inasistencia perdía fuerza, y que su significado (3 casos en el historial del paciente de prueba) no tenía impacto en el diseño.

  Los dos errores:

  1. **La magnitud.** Los 3 casos eran del paciente de prueba, no de la base. En el catálogo vivo (bloque 4) el estado `2` son **158.155 filas, el 14,6 %** de la agenda histórica. Y `NA` en `CITAS_ANULADAS` son **285**. Dos órdenes de magnitud separan una cosa de la otra: lejos de restarle fuerza, la comparación *refuerza* que `NA` es "avisó que no venía" y el `2` es el no-show masivo.
  2. **El impacto.** "El agente reacciona igual ante cualquier UPDATE" solo es cierto para *detectar* el cambio. Para *reportarlo* no: `desenlaceDeAtencion()` devuelve `null` para el `2`, así que el desenlace de ese 14,6 % nunca llega a AgenIA. Y en sentido contrario, `updateAttendance()` (AgenIA→HIS) no se puede implementar sin saber qué valor escribe un no-show.

  **Estado real: pregunta abierta, no cerrada.** Las consultas que la deciden están en `sql/PENDIENTE_CORRER_EN_HOSPITAL.sql` **sección I** (ocho, todas de lectura, contra `ESEHSVP`). La que puede tumbar la hipótesis es I.5: si las citas en estado `2` están facturadas a un convenio igual que las de estado `1`, alguien las atendió y el `2` significa otra cosa.
  - **Muestra fila-a-fila obtenida** (2026-08-23): confirma formato — `TX_OBSE_CIAN` en la práctica suele venir vacío o con puntos de relleno (`"...."`, `"SSSSS"` — placeholders de digitación del personal, no texto significativo); `CD_CODI_MOTI_CIAN='05'` domina la muestra reciente, consistente con la distribución histórica. ✅ **Resuelto (I.8, 2026-09-07): `05` = "PACIENTE LLAMA A CANCELAR"**, con 7.087 de las ~8.301 anulaciones del último año (85,4 %). Cierra el pendiente 0b. Nota de negocio, no técnica: son 7.000 pacientes al año llamando por teléfono a cancelar — exactamente el volumen que el chatbot puede absorber. Y `MOTIVOANUL` resultó ser un catálogo **compartido** con la anulación de cargos de facturación ("NO POS", "COPAGO NO COBRADO", "DEVOLUCION DINERO"), así que no todos sus códigos aplican a una cita.
